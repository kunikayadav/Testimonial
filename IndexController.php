...
class TM_Testimonials_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function postAction()
    {
        ...
        if ($data = $this->getRequest()->getPost()) {
			...
			// upload file
			 if( isset($_FILES['text_file']['name']) && ($_FILES['text_file']['tmp_name'] != NULL) ){
            try {
                $uploader = new Varien_File_Uploader('text_file');
                $uploader->setAllowedExtensions(array('txt'));
                $uploader->setAllowRenameFiles(true);
                $uploader->setFilesDispersion(false);
                $path = Mage::getBaseDir('media') . TM_Testimonials_Model_Data::FILE_PATH; // where we save images
                $result = $uploader->save($path, $_FILES['text_file']['name']);
            } catch (Exception $e) {
                echo $e->getMessage();
            }
        }
